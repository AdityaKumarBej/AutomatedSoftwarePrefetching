...
struct SwPrefetchPass : FunctionPass, InstVisitor<SwPrefetchPass>
{
    ...
    bool makeLoopInvariantSpec(Instruction *I, bool &Changed, Loop* L) const {
        // Enhanced: Explicit check for null pointers
        if (!I || !L) return false;
        ...
        if (canHoist) {
            ...
            Changed = true; // Indicate that the instruction was hoisted
            return true;
        }
   ...
   return true;
    ...
    Value *getArrayOrAllocSize(LoadInst* l) const {
        ...
        if (!l) return NULL; // Early null check
    ...
    Value *getCanonicalishSizeVariable(Loop* L) const {
        ...
        if(!L) return nullptr; // Early null check
    ...
    GetElementPtrInst *getWeirdCanonicalishInductionVariableGep(Loop* L) const {
        if (!L) return nullptr; // Early null check for Loop

        BasicBlock *H = L->getHeader();
        if (!H) return nullptr; // Early null check for Header

        BasicBlock *Incoming = nullptr, *Backedge = nullptr;
        pred_iterator PI = pred_begin(H);
        assert(PI != pred_end(H) && "Loop must have at least one backedge!");
        Backedge = *PI++;
        if (PI == pred_end(H)) return nullptr; // Dead loop check
        Incoming = *PI++;
        if (PI != pred_end(H)) return nullptr; // Multiple backedges check

        if (!isValidBackedge(L, Incoming, Backedge)) return nullptr;

        // Enhanced: Streamlined loop over PHINodes
    ...
    // Enhanced: New helper function to check backedge validity
    bool isValidBackedge(const Loop* L, BasicBlock* Incoming, BasicBlock* Backedge) const {
        if (L->contains(Incoming)) {
            if (L->contains(Backedge)) return false;
            std::swap(Incoming, Backedge);
        } else if (!L->contains(Backedge)) return false;
        return true;
    }

    // Enhanced: New helper function to validate GEP induction variable
    ...
    Value *getWeirdCanonicalishInductionVariableFirst(Loop* L) const {
        // Early null check for Loop and Header
        ...
        // Simplified backedge determination and validation
        Backedge = *PI++;
        Incoming = (PI != pred_end(H)) ? *PI++ : nullptr;
        if (!Incoming || PI != pred_end(H) || !L->contains(Backedge)) return nullptr;

        // Streamlined loop over PHINodes
        for (PHINode &PN : H->phis()) {
            GetElementPtrInst *Inc = dyn_cast<GetElementPtrInst>(PN.getIncomingValueForBlock(Backedge));
            if (Inc && isValidGEPInductionVariable(Inc, &PN, L, Incoming)) {
    ...
    Value *getOddPhiFirst(Loop* L, PHINode* PN) const {
        // Early null checks for Loop and PHINode
        if (!L || !PN || !L->getHeader()) return nullptr;
        ...
        // Check if PHINode's parent is the header
        ...
        for(auto &BB : F) {
            for (auto &I : BB) {
                if (LoadInst *i = dyn_cast<LoadInst>(&I)) {
                    if(LI.getLoopFor(&BB)) {
                        // Enhanced: Code refactoring for depth-first search
                        processLoadInst(i, LI, Loads, Phis, Insts, modified);
                    }
                }
            }
...
z
        ...
        // Enhanced: Refactored loop for processing Loads
        processLoads(LI, Loads, Phis, Insts, modified);

        return modified;
    }

    void processLoadInst(LoadInst *i, LoopInfo &LI, SmallVector<Instruction*,4> &Loads, SmallVector<Instruction*,4> &Phis, std::vector<SmallVector<Instruction*,8>> &Insts, bool &modified) {
    SmallVector<Instruction*,8> Instrz;
    Instrz.push_back(i);
    Instruction *phi = nullptr;

    if (depthFirstSearch(i, LI, phi, Instrz, Loads, Phis, Insts)) {
        int loadCount = std::count_if(Instrz.begin(), Instrz.end(), [](Instruction* z) { return isa<LoadInst>(z); });

        if (loadCount < 2) {
            dbgs() << "Stride detected for " << *i << ". Skipping stride cases.\n";
#ifdef NO_STRIDES
            return;
#endif
        }
        ...
        dbgs() << "Cannot prefetch load " << *i << "\n";
    }
}

void processLoads(LoopInfo &LI, SmallVector<Instruction*,4> &Loads, SmallVector<Instruction*,4> &Phis, std::vector<SmallVector<Instruction*,8>> &Insts, bool &modified) {
    for (size_t x = 0; x < Loads.size(); ++x) {
        ValueMap<Instruction*, Value*> Transforms;
        bool ignore = true;

        Loop* L = LI.getLoopFor(Phis[x]->getParent());

        for (size_t y = x + 1; y < Loads.size(); ++y) {
            bool subset = std::all_of(Insts[x].begin(), Insts[x].end(), [&Insts, y](Instruction* in) { return std::find(Insts[y].begin(), Insts[y].end(), in) != Insts[y].end(); });
        ...
        // Additional logic for prefetch transformations
        // ...
        // This section will contain the transformation logic for each load
        // based on the analysis and modifications done above.
    }
}
