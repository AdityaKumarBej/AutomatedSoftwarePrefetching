...
if [ "${CK_COMPILE_TYPE}" == "auto" ] ; then
  echo "*** auto ***"
  EXECUTE1="$CK_COMPILER ./camel.c -DPREFETCH=1 -DNUMHASH=${CK_CAMEL_HASHES}  -Xclang -load -Xclang ${CK_ENV_PLUGIN_LLVM_SW_PREFETCH_PASS_FILE} -c -S -emit-llvm"
  ...
  echo "*** man (64) ***"
  EXECUTE1="$CK_COMPILER ./camel.c -DPREFETCH=1 -DNUMHASH=${CK_CAMEL_HASHES}  -DFETCHDIST=32 -DSTRIDE -c"
  EXECUTE2=""
  EXECUTE3="$CK_COMPILER camel.o ${CK_NAS_COMMON}/c_print_results.c ${CK_NAS_COMMON}/c_timers.c ${CK_NAS_COMMON}/wtime.c -lm"
