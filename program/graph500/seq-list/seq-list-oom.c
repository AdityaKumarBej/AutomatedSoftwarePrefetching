...
create_graph_from_edgelist (struct packed_edge *IJ_in, int64_t nedge)
{
  ...
  for (k = 0; k < nedge; ++k) {
    int64_t prefetch_index = (k + PREFETCH_DISTANCE) % nedge; // Ensure it's within bounds
    __builtin_prefetch(&IJ_in[prefetch_index]);
	       ...
	       int64_t srcvtx)
{
  ...
  *max_vtx_out = maxvtx;
  ...
  while (k1 != k2) {
    int64_t k, newk2 = k2;
    for (k = k1; k < k2; ++k) {
        // Non-sequential prefetch for vertices
      int64_t prefetch_index = (k + PREFETCH_DISTANCE) % nv; // Ensure it's within bounds
      __builtin_prefetch(&vlist[prefetch_index]);
      __builtin_prefetch(&bfs_tree[vlist[prefetch_index]]);
