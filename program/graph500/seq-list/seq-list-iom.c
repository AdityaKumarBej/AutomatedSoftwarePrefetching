...
create_graph_from_edgelist (struct packed_edge *IJ_in, int64_t nedge)
{
  ...
  for (k = 0; k < nedge; ++k) {
      ...
      __builtin_prefetch(&IJ_in[k + PREFETCH_DISTANCE]);
    }
	       ...
	       int64_t srcvtx)
{
  ...
  *max_vtx_out = maxvtx;
  ...
  while (k1 != k2) {
    int64_t k, newk2 = k2;
    for (k = k1; k < k2; ++k) {
      // Prefetch for the next set of vertices
      if (k + PREFETCH_DISTANCE < k2) {
        __builtin_prefetch(&vlist[k + PREFETCH_DISTANCE]);
        __builtin_prefetch(&bfs_tree[vlist[k + PREFETCH_DISTANCE]]);
      }
