...
gather_edges (const struct packed_edge * restrict IJ, int64_t nedge)
{
  OMP("omp parallel") {
    ...
    OMP("omp for")
      for (k = 0; k < nedge; ++k) {
        // Out-of-order prefetch for upcoming IJ entries
        if (k + PREFETCH_DISTANCE < nedge) {
            __builtin_prefetch(&IJ[k + PREFETCH_DISTANCE], 0, PREFETCH_LEVEL);
        }
        int64_t i = get_v0_from_edge(&IJ[k]);
        int64_t j = get_v1_from_edge(&IJ[k]);
	if (i >= 0 && j >= 0 && i != j) {
...
#define THREAD_BUF_LEN 16384
  ...
  OMP("omp parallel shared(k1, k2)") {
    ...
    while (k1 != k2) {
      ...
      OMP("omp for")
	for (k = k1; k < oldk2; ++k) {
// Out-of-order prefetch for next iterations
    if (k + PREFETCH_DISTANCE < oldk2) {
        __builtin_prefetch(&vlist[k + PREFETCH_DISTANCE], 0, PREFETCH_LEVEL);
        // Prefetch related xadj entries
        __builtin_prefetch(&xadj[XOFF(vlist[k + PREFETCH_DISTANCE])], 0, PREFETCH_LEVEL);
    }
